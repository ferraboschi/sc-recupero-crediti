"""Test della pagina "Utilizzo": contatore di solleciti per giorno.

Sola lettura: l'endpoint organizza i solleciti registrati (RecoveryAction con
un `channel` valorizzato, non annullati) in due viste — l'elenco degli eventi
e il conteggio giornaliero dei clienti DISTINTI sollecitati. La "data" è il
giorno di calendario ITALIANO di `created_at` (non il giorno UTC nudo).
"""
...
class TestUtilizzoEventi:
    def test_lists_sollecito_with_customer_name_and_italian_date(
        ...
        # 14 gennaio 23:30 UTC → in Italia (UTC+1 d'inverno) è già il 15
    ...
    def test_only_channel_valorized_counts(self, test_client, test_db_session):
        # Un'azione senza channel (es. un todo 'wait') NON è un sollecito
        # inviato e non deve comparire.
        c = _customer(test_db_session, "Rooftop S.R.L.")
...
class TestUtilizzoPerGiorno:
    def test_counts_distinct_customers_per_day(
        ...
        assert giorno["clienti_sollecitati"] == 2  # DISTINTI
        assert giorno["eventi_totali"] == 3
...
class TestUtilizzoAnnullati:
    def test_excludes_cancelled(self, test_client, test_db_session):
        ...
        data = test_client.get("/api/recovery/utilizzo").json()
        assert len(data["eventi"]) == 1
        assert data["per_giorno"][0]["clienti_sollecitati"] == 1
        assert data["per_giorno"][0]["eventi_totali"] == 1
